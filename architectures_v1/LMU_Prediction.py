...
class LMU_Prediction:

    def __init__(self, input_shape, dataset,
                 units, order, build = False):
        ...
        self.units = units
        self.order = order
        self.theta = self.length
        self.input_encoders_initializer = Constant(1)
        self.hidden_encoders_initializer = Constant(0)
        self.memory_encoders_initializer = Constant(0)
        self.input_kernel_initializer = Constant(0)
        self.hidden_kernel_initializer = Constant(0)
        self.memory_kernel_initializer = "glorot_normal"
    ...
    def build_model(self):
        ...
        model.add(RNN(LMUCell(units = self.units,
            order = self.order,
            theta = self.theta,
            ...
            memory_kernel_initializer = self.memory_kernel_initializer),
                      input_shape = (self.length, self.n_features), return_sequences = False))
